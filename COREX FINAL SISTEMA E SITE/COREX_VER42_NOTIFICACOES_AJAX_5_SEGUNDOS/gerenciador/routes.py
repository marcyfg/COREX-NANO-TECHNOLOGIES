...
SETOR_SERVICE_DESK = "Service Desk / Triagem"
...
def migrar_banco_sem_perder_dados():
    ...
    with app.app_context():
        ...
        # VER36: principal_admin foi removido. O campo cargo define toda a hierarquia.
        if "principal_admin" in colunas_usuario:
            try:
                database.session.execute(text("ALTER TABLE usuario DROP COLUMN principal_admin"))
        ...
        if primeiro_usuario:
            ...
            primeiro_usuario.cargo = "ADMIN_SUPERVISOR"
        ...
        for usuario in Usuario.query.all():
            ...
            # Migração automática do modelo antigo admin para cargos profissionais.
            cargo_atual = getattr(usuario, "cargo", None)
            if usuario.id == 1 or cargo_atual == "ADMIN_SUPERVISOR":
                usuario.cargo = "ADMIN_SUPERVISOR"
            elif getattr(usuario, "admin", False) and cargo_atual not in ["LIDER", "TECNICO_COREX"]:
                usuario.cargo = "LIDER"
            elif cargo_atual not in ["ADMIN_SUPERVISOR", "LIDER", "TECNICO_COREX", "CLIENTE"]:
                usuario.cargo = "CLIENTE"

            if usuario.cargo == "ADMIN_SUPERVISOR":
                usuario.admin = True
            elif usuario.cargo == "LIDER":
                usuario.admin = True
            else:
                usuario.admin = False
...
def cargo_usuario(usuario):
    ...
    # Compatibilidade com versões antigas: Super Admin/Admin CoreX viram Administrador Supervisor.
    if cargo in ["SUPER_ADMIN", "ADMIN_COREX", "ADMIN_SUPERVISOR"]:
        return "ADMIN_SUPERVISOR"
    if cargo in ["SERVICE_DESK", "LIDER", "TECNICO_COREX", "CLIENTE"]:
        return cargo
    return "ADMIN_SUPERVISOR" if getattr(usuario, "admin", False) else "CLIENTE"


def sincronizar_flags_usuario(usuario):
    cargo = cargo_usuario(usuario)
    usuario.cargo = cargo
    usuario.admin = cargo in ["ADMIN_SUPERVISOR", "LIDER"]


def is_adm_principal(usuario):
    return bool(usuario.is_authenticated and cargo_usuario(usuario) == "ADMIN_SUPERVISOR")


def is_admin_corex(usuario):
    return bool(usuario.is_authenticated and cargo_usuario(usuario) in ["ADMIN_SUPERVISOR", "LIDER"])


def is_service_desk(usuario):
    return bool(usuario.is_authenticated and cargo_usuario(usuario) == "SERVICE_DESK")
...
def is_equipe_corex(usuario):
    return bool(usuario.is_authenticated and cargo_usuario(usuario) in ["ADMIN_SUPERVISOR", "SERVICE_DESK", "LIDER", "TECNICO_COREX"])


def is_lider_corex(usuario):
    return bool(usuario.is_authenticated and cargo_usuario(usuario) == "LIDER")


def tem_dashboard(usuario):
    return bool(usuario.is_authenticated and cargo_usuario(usuario) in ["ADMIN_SUPERVISOR", "LIDER"])


def rotulo_fila_usuario(usuario):
    cargo = cargo_usuario(usuario)
    if cargo in ["ADMIN_SUPERVISOR", "SERVICE_DESK"]:
        return "Fila Geral"
    if cargo == "LIDER":
        return "Chamados do Setor"
    if cargo == "TECNICO_COREX":
        return "Meus Chamados"
    return "Meus Chamados"


def pode_ver_fila(usuario):
    return bool(usuario.is_authenticated)
...
def lider_responde_pelo_chamado(tarefa):
    """Líder enxerga e gerencia chamados do próprio setor.

    Além do setor gravado no chamado, também considera o setor do técnico responsável.
    Isso evita bloquear o líder quando o chamado está atribuído a um técnico do setor dele.
    """
    if cargo_usuario(current_user) != "LIDER":
        return False

    setor_lider = setor_usuario(current_user)
    if normalizar_setor(getattr(tarefa, "setor", None)) == setor_lider:
        return True

    responsavel = getattr(tarefa, "usuario", None)
    if responsavel and setor_usuario(responsavel) == setor_lider:
        return True
...
def pode_gerenciar_tarefa(tarefa):
    ...
    if cargo == "ADMIN_SUPERVISOR":
        return True
    if cargo == "SERVICE_DESK":
        return (
            tarefa.id_usuario == current_user.id
            or tarefa.status in ["aberto", "aguardando_reclassificacao"]
            or normalizar_setor(getattr(tarefa, "setor", None)) in [SETOR_PADRAO, SETOR_SERVICE_DESK]
        )
    if cargo == "LIDER":
        return lider_responde_pelo_chamado(tarefa)
    if cargo == "TECNICO_COREX":
        # Técnico CoreX pode atuar apenas no chamado atribuído a ele: comentar, atualizar andamento e finalizar.
        # Ações administrativas sensíveis ficam separadas em helpers próprios.
        return tarefa.id_usuario == current_user.id
    return False


def pode_acao_admin_tarefa(tarefa):
    """Ações administrativas sensíveis do chamado.

    Usado para Encerrar admin, Reabrir e ações de alto impacto.
    Técnico CoreX não entra aqui, mesmo quando o chamado está atribuído a ele.
    """
    return is_admin_corex(current_user) and pode_gerenciar_tarefa(tarefa)


def pode_editar_tarefa(tarefa):
    """Permite corrigir dados estruturais do chamado.

    O Service Desk precisa editar chamados para completar a triagem: categoria,
    prioridade, local do problema, setor técnico e responsável.
    Líder continua podendo editar chamados do próprio setor.
    """
    cargo = cargo_usuario(current_user)
    return cargo in ["ADMIN_SUPERVISOR", "SERVICE_DESK", "LIDER"] and pode_gerenciar_tarefa(tarefa)


def valor_setor_cliente_form(form):
    opcao = (getattr(form, "setor_cliente_opcao").data or "").strip()
    outros = (getattr(form, "setor_cliente_outros").data or "").strip()
    if opcao == "Outros":
        if not outros:
            flash("Informe o setor/local do problema quando selecionar Outros.", "erro")
            return None
        return outros
    return opcao or None


def preencher_setor_cliente_form(form, valor):
    valor = (valor or "").strip()
    opcoes = {v for v, _ in SETORES_LOCAL_PROBLEMA}
    if valor in opcoes and valor != "Outros":
        form.setor_cliente_opcao.data = valor
        form.setor_cliente_outros.data = ""
    elif valor:
        form.setor_cliente_opcao.data = "Outros"
        form.setor_cliente_outros.data = valor
    else:
        form.setor_cliente_opcao.data = ""
        form.setor_cliente_outros.data = ""
...
def pode_visualizar_tarefa(tarefa):
    ...
    if cargo == "ADMIN_SUPERVISOR":
        return True
    if cargo == "SERVICE_DESK" and (
        tarefa.id_usuario == current_user.id
        or tarefa.status in ["aberto", "aguardando_reclassificacao"]
        or normalizar_setor(getattr(tarefa, "setor", None)) in [SETOR_PADRAO, SETOR_SERVICE_DESK]
    ):
        return True
    if cargo == "LIDER" and lider_responde_pelo_chamado(tarefa):
        return True
    if cargo == "TECNICO_COREX" and (
        tarefa.id_usuario == current_user.id
        or tarefa.id_solicitante == current_user.id
        or (tarefa.id_usuario is None and normalizar_setor(getattr(tarefa, "setor", None)) == setor_usuario(current_user) and not tarefa.concluida and not tarefa.encerrada_forcada)
    ):
        return True
...
def pode_assumir_tarefa(tarefa):
    ...
    cargo = cargo_usuario(current_user)
    setor_tarefa = normalizar_setor(getattr(tarefa, "setor", None))

    # Administrador Supervisor e Service Desk atuam na triagem da Fila Geral.
    if cargo in ["ADMIN_SUPERVISOR", "SERVICE_DESK"]:
        return setor_tarefa in [SETOR_PADRAO, SETOR_SERVICE_DESK] or tarefa.status == "aguardando_reclassificacao"

    # Líder e Técnico assumem somente chamados já direcionados ao setor deles.
    if cargo in ["LIDER", "TECNICO_COREX"]:
        return setor_tarefa == setor_usuario(current_user)

    return False


def pode_transferir_tarefa(tarefa):
    # Técnico CoreX também pode transferir, mas somente dentro das regras de setor/técnicos.
    return is_equipe_corex(current_user) and pode_gerenciar_tarefa(tarefa)
...
def aplicar_filtros_tarefas(consulta):
    ...
    if is_adm_principal(current_user):
    ...
    elif cargo_usuario(current_user) == "LIDER":
        # Líder vê a operação do próprio setor: chamados do setor e chamados atribuídos
        # aos técnicos/líderes do setor dele. A Fila Geral de triagem fica separada.
        consulta = consulta.outerjoin(Usuario, Tarefa.id_usuario == Usuario.id).filter(
            or_(
        ...
        # Técnico vê os chamados atribuídos a ele e chamados que ele abriu.
        # A fila do setor é tratada separadamente na rota feed.
        consulta = consulta.filter(or_(Tarefa.id_usuario == current_user.id, Tarefa.id_solicitante == current_user.id))
    else:
...
def cargos_permitidos_transferencia():
    """Define quem pode aparecer no campo Transferir para.

    Regras:
    - Técnico CoreX transfere somente para outros Técnicos CoreX.
    - Líder transfere para Técnicos CoreX e Líderes.
    - Administrador Supervisor pode transferir para qualquer cargo interno CoreX.
    - Cliente nunca aparece como responsável técnico.
    """
    if is_adm_principal(current_user):
        return ["SERVICE_DESK", "LIDER", "TECNICO_COREX"]
    if cargo_usuario(current_user) == "SERVICE_DESK":
        return ["LIDER", "TECNICO_COREX"]
    if cargo_usuario(current_user) == "LIDER":
        return ["LIDER", "TECNICO_COREX"]
    if cargo_usuario(current_user) == "TECNICO_COREX":
        return ["TECNICO_COREX"]
    return []


def responsaveis_corex_do_escopo(setor=None, respeitar_setor=True):
    """Lista responsáveis possíveis respeitando cargo e setor selecionado.

    A lista agora sempre respeita o setor escolhido na transferência. Assim,
    ao escolher "Laboratório de Informática", aparecem apenas usuários daquele setor.
    """
    cargos = cargos_permitidos_transferencia()
    if not cargos:
        return []

    consulta = Usuario.query.filter(
        Usuario.ativo == True,
        Usuario.cargo.in_(cargos)
    )

    if setor:
        consulta = consulta.filter_by(setor=normalizar_setor(setor))
    elif not is_adm_principal(current_user) and not is_service_desk(current_user):
        consulta = consulta.filter_by(setor=setor_usuario(current_user))

    return consulta.order_by(Usuario.setor.asc(), Usuario.cargo.asc(), Usuario.username.asc()).all()


def comentario_obrigatorio():
    texto = (request.form.get("comentario") or request.form.get("texto") or "").strip()
    if not texto:
        flash("Para realizar essa ação, informe um comentário.", "erro")
    return texto
...
def notificar_admins_do_setor(setor, titulo, mensagem, tarefa=None):
    admins = Usuario.query.filter(Usuario.ativo == True, Usuario.cargo.in_(["ADMIN_SUPERVISOR", "LIDER"])).all()
    for admin in admins:
        if is_adm_principal(admin) or setor_usuario(admin) == normalizar_setor(setor):
            criar_notificacao(admin, titulo, mensagem, tarefa)


def notificar_triagem_service_desk(titulo, mensagem, tarefa=None, ignorar_usuario_id=None):
    """Notifica somente quem faz triagem da Fila Geral: Service Desk e Administrador Supervisor."""
    equipe = Usuario.query.filter(
        Usuario.ativo == True,
        Usuario.cargo.in_(["ADMIN_SUPERVISOR", "SERVICE_DESK"])
    ).all()
...
def notificar_lideres_do_setor(setor, titulo, mensagem, tarefa=None):
    ...
    for lider in lideres:
        criar_notificacao(lider, titulo, mensagem, tarefa)
...
def configurar_form_tarefa(form):
    ...
    # Responsáveis seguem exatamente o setor selecionado.
    responsaveis = responsaveis_corex_do_escopo(setor=setor_escolhido) if is_equipe_corex(current_user) else []
...
def inject_corex_context():
    ...
    return dict(
        ...
        is_lider_corex=is_lider_corex,
        is_service_desk=is_service_desk,
        tem_dashboard=tem_dashboard,
        rotulo_fila_usuario=rotulo_fila_usuario,
        pode_ver_fila=pode_ver_fila,
        cargo_usuario=cargo_usuario,
        rotulo_cargo=rotulo_cargo,
        pode_gerenciar_usuario=pode_gerenciar_usuario,
        pode_acao_admin_tarefa=pode_acao_admin_tarefa,
...
def garantir_banco_e_admin_padrao():
    ...
    if not admin:
        ...
        if not admin.admin:
            admin.admin = True
            alterou = True
        if getattr(admin, "cargo", None) != "ADMIN_SUPERVISOR":
...
def criarconta():
    ...
    if not is_adm_principal(current_user):
        form.cargo.choices = [("TECNICO_COREX", "Técnico CoreX")]
        form.cargo.data = "TECNICO_COREX"
    ...
    if form.validate_on_submit():
        ...
        if not is_adm_principal(current_user) and cargo not in ["TECNICO_COREX"]:
            abort(403)
        # Administrador Supervisor pode criar qualquer tipo de cargo.
        criar_como_admin = cargo in ["ADMIN_SUPERVISOR", "LIDER"]
...
def usuarios():
    ...
    filtro_cargo = request.args.get("cargo") or "todos"
    filtro_setor = request.args.get("setor") or "todos"
    busca = (request.args.get("q") or "").strip()
    ...
    if is_adm_principal(current_user):
        ...
        consulta = consulta.filter(Usuario.setor == setor_usuario(current_user), Usuario.cargo.in_(["TECNICO_COREX", "CLIENTE"]))

    if filtro_cargo != "todos":
        consulta = consulta.filter(Usuario.cargo == filtro_cargo)
    if filtro_setor != "todos" and is_adm_principal(current_user):
        consulta = consulta.filter(Usuario.setor == normalizar_setor(filtro_setor))
    if busca:
        like = f"%{busca}%"
        consulta = consulta.filter(or_(Usuario.username.ilike(like), Usuario.email.ilike(like), Usuario.empresa.ilike(like), Usuario.cnpj.ilike(like)))

    lista_usuarios = consulta.order_by(Usuario.cargo.asc(), Usuario.setor.asc(), Usuario.username.asc()).all()
...
def editar_usuario(id_usuario):
    ...
    if form.validate_on_submit():
        ...
        if is_adm_principal(current_user):
            ...
            if usuario.id == 1 or novo_cargo == "ADMIN_SUPERVISOR":
                novo_cargo = "ADMIN_SUPERVISOR" if usuario.id == 1 else "LIDER"
...
def gerenciador_interno():
    ...
    # - Administrador Supervisor: visão geral da operação e triagem.
    # - Líder: visão analítica somente do próprio setor.
    # Técnico e Cliente usam Fila/Meus Chamados.
    if not tem_dashboard(current_user):
        flash('Dashboard é uma área de análise para Supervisores e Líderes. Use a fila de chamados para acompanhar atendimentos.', 'info')
        return redirect(url_for('feed'))
    ...
    if is_adm_principal(current_user):
        ...
        base_stats = base_stats.outerjoin(Usuario, Tarefa.id_usuario == Usuario.id).filter(
            or_(Tarefa.setor == setor_usuario(current_user), Usuario.setor == setor_usuario(current_user))
        )
    elif cargo_usuario(current_user) == "TECNICO_COREX":
        base_stats = base_stats.filter(or_(Tarefa.id_usuario == current_user.id, Tarefa.id_solicitante == current_user.id))
    else:
...
def abrir_chamado():
    ...
    if form.validate_on_submit():
        setor_cliente_valor = valor_setor_cliente_form(form)
        if setor_cliente_valor is None:
        ...
        if is_equipe_corex(current_user):
            ...
            setor_tarefa = SETOR_SERVICE_DESK
        ...
        if usuario_destino:
            if not getattr(usuario_destino, "ativo", True) or cargo_usuario(usuario_destino) not in ["LIDER", "TECNICO_COREX"]:
                abort(403)
            if not is_adm_principal(current_user) and setor_usuario(usuario_destino) != setor_tarefa:
                abort(403)
            # Super Admin pode escolher responsável de qualquer setor; a fila técnica acompanha o responsável.
            if is_adm_principal(current_user):
                setor_tarefa = setor_usuario(usuario_destino)
        ...
        tarefa = Tarefa(
            ...
            prioridade=(form.prioridade.data if is_equipe_corex(current_user) else "Média"),
            status="aberto",
            imagem=nome_imagem,
            id_usuario=usuario_destino.id if usuario_destino else None,
            id_solicitante=current_user.id,
            setor=setor_tarefa,
            setor_cliente=setor_cliente_valor
        )
        database.session.add(tarefa)
        database.session.flush()
        tarefa.numero_ticket = gerar_numero_ticket(tarefa)
        if is_equipe_corex(current_user):
            ...
            if not usuario_destino:
                notificar_lideres_do_setor(setor_tarefa, "Novo chamado encaminhado ao setor", f"{tarefa.numero_ticket} - {tarefa.titulo}", tarefa)
        else:
            ...
            notificar_triagem_service_desk(
...
@login_required
def feed():
    # Visão operacional por perfil:
    # Administrador Supervisor: Fila Geral de triagem, somente chamados novos sem setor e sem responsável.
    # Líder: Chamados do Setor, incluindo sem responsável, em atendimento por técnicos do setor e concluídos pelo setor.
    # Técnico: Meus Chamados, incluindo chamados atribuídos a ele e chamados disponíveis sem responsável no setor.
    # Cliente: Meus chamados da própria empresa/CNPJ.
    cargo = cargo_usuario(current_user)
    status = request.args.get("status") or "todos"

    if status == "concluidos":
        consulta = Tarefa.query.filter(Tarefa.concluida == True, Tarefa.encerrada_forcada == False)
    elif status == "forcados":
        consulta = Tarefa.query.filter(Tarefa.encerrada_forcada == True)
    else:
        consulta = Tarefa.query.filter(Tarefa.concluida == False, Tarefa.encerrada_forcada == False)

    if cargo in ["ADMIN_SUPERVISOR", "SERVICE_DESK"]:
        # Fila Geral é triagem: chamados novos ou devolvidos para reclassificação.
        consulta = consulta.filter(
            Tarefa.id_usuario == None,
            or_(Tarefa.setor == SETOR_PADRAO, Tarefa.setor == SETOR_SERVICE_DESK, Tarefa.status == "aguardando_reclassificacao")
        )
    elif cargo == "LIDER":
        # Líder acompanha toda a operação do próprio setor, inclusive chamados já assumidos
        # pelos técnicos do setor.
        consulta = consulta.outerjoin(Usuario, Tarefa.id_usuario == Usuario.id).filter(
            or_(
                Tarefa.setor == setor_usuario(current_user),
                Usuario.setor == setor_usuario(current_user)
    ...
    elif cargo == "TECNICO_COREX":
        # Técnico vê os próprios chamados e os chamados sem responsável disponíveis no setor dele.
        consulta = consulta.filter(
            or_(
                Tarefa.id_usuario == current_user.id,
                and_(Tarefa.id_usuario == None, Tarefa.setor == setor_usuario(current_user))
            )
        )
    else:
        consulta = consulta.filter(filtro_historico_cliente(current_user))
    ...
    if cargo == "ADMIN_SUPERVISOR" and setor_filtro != "todos":
        consulta = consulta.filter(Tarefa.setor == normalizar_setor(setor_filtro))

    if status == "sem_responsavel":
        consulta = consulta.filter(Tarefa.id_usuario == None)
    elif status == "em_andamento":
        consulta = consulta.filter(or_(Tarefa.status == "em_andamento", Tarefa.id_usuario != None))
    elif status == "aguardando_cliente":
        consulta = consulta.filter(Tarefa.status == "aguardando_cliente")
...
def pode_devolver_service_desk(tarefa):
    cargo = cargo_usuario(current_user)
    if cargo == "ADMIN_SUPERVISOR":
        return True
    if cargo == "LIDER":
        return lider_responde_pelo_chamado(tarefa)
    if cargo == "TECNICO_COREX":
        return tarefa.id_usuario == current_user.id or (normalizar_setor(getattr(tarefa, "setor", None)) == setor_usuario(current_user) and not tarefa.concluida and not tarefa.encerrada_forcada)
    return False
...
def detalhe_tarefa(id_tarefa):
    ...
    if pode_transferir_tarefa(tarefa):
        ...
        usuarios_transferencia = [
            {"id": u.id, "nome": u.username, "setor": setor_usuario(u), "cargo": cargo_usuario(u), "cargo_nome": rotulo_cargo(u)}
            for u in responsaveis_corex_do_escopo(setor=None)
        ]
    else:
    ...
    return render_template(
        ...
        pode_acao_admin=pode_acao_admin_tarefa(tarefa),
        pode_assumir=pode_assumir_tarefa(tarefa),
        pode_transferir=pode_transferir_tarefa(tarefa),
        usuarios_transferencia=usuarios_transferencia,
        pode_devolver_service_desk=pode_devolver_service_desk(tarefa)
    )
...
def editar_tarefa(id_tarefa):
    ...
    # Service Desk também pode editar para completar a triagem do chamado.
    # Técnico pode comentar e mudar andamento, mas não reescrever a solicitação original.
    if not pode_editar_tarefa(tarefa):
        abort(403)
    ...
    if request.method == "GET":
        ...
        preencher_setor_cliente_form(form, getattr(tarefa, "setor_cliente", None))
    ...
    if form.validate_on_submit():
        ...
        tarefa.setor_cliente = setor_cliente_valor
...
def transferir_tarefa(id_tarefa):
    ...
    # Todos os cargos internos podem direcionar para outro setor, mas a lista de pessoas
    # precisa respeitar o setor escolhido e o cargo permitido para quem está transferindo.
    ...
    if form.validate_on_submit():
        ...
        if novo_usuario:
            # Cliente nunca recebe chamado como responsável técnico.
            # O usuário escolhido precisa pertencer ao setor selecionado e ao cargo permitido.
            if (not novo_usuario.ativo) or cargo_usuario(novo_usuario) not in cargos_permitidos_transferencia():
                abort(403)
            if setor_usuario(novo_usuario) != setor_destino:
                flash("O responsável escolhido não pertence ao setor selecionado.", "erro")
                return redirect(url_for('detalhe_tarefa', id_tarefa=tarefa.id))
        ...
        # Notifica o novo responsável, ou o líder do setor quando o chamado foi encaminhado sem técnico.
        if novo_usuario:
            criar_notificacao(novo_usuario, "Chamado atribuído a você", f"{tarefa.numero_ticket or tarefa.id} - {tarefa.titulo}", tarefa)
        elif setor_destino not in [SETOR_PADRAO, SETOR_SERVICE_DESK]:
            notificar_lideres_do_setor(setor_destino, "Chamado encaminhado ao seu setor", f"{tarefa.numero_ticket or tarefa.id} - {tarefa.titulo}", tarefa)
        database.session.commit()
...
@app.route('/tarefa/<int:id_tarefa>/devolver-service-desk', methods=['POST'])
@login_required
def devolver_service_desk(id_tarefa):
    tarefa = Tarefa.query.get_or_404(id_tarefa)
    if not pode_devolver_service_desk(tarefa):
        abort(403)

    comentario = comentario_obrigatorio()
    if not comentario:
        return redirect(url_for('detalhe_tarefa', id_tarefa=tarefa.id))
    ...
    tarefa.setor = SETOR_SERVICE_DESK
    tarefa.id_usuario = None
    atualizar_status_tarefa(tarefa, "aguardando_reclassificacao")

    registrar_comentario(tarefa, f"Chamado devolvido para o Service Desk por {current_user.username}: setor anterior {antigo_setor}; responsável anterior {antigo_resp}.")
    registrar_comentario(tarefa, comentario)
    notificar_triagem_service_desk("Chamado devolvido para reclassificação", f"{tarefa.numero_ticket or tarefa.id} - {tarefa.titulo}", tarefa, ignorar_usuario_id=current_user.id)

    database.session.commit()
    flash("Chamado devolvido para o Service Desk com sucesso.", "ok")
    return redirect(url_for('detalhe_tarefa', id_tarefa=tarefa.id))
...
@app.route('/forcar-encerramento/<int:id_tarefa>', methods=['POST'])
@login_required
def forcar_encerramento(id_tarefa):
    tarefa = Tarefa.query.get_or_404(id_tarefa)
    if not pode_acao_admin_tarefa(tarefa):
        abort(403)
    comentario = comentario_obrigatorio()
    if not comentario:
        return redirect(url_for('detalhe_tarefa', id_tarefa=tarefa.id))
...
@app.route('/reabrir-tarefa/<int:id_tarefa>', methods=['POST'])
@login_required
...
@app.route('/tarefa/<int:id_tarefa>/status/<novo_status>', methods=['POST'])
@login_required
...
def contador_notificacoes():
    """Retorna somente a quantidade de notificações não lidas do usuário logado."""
    quantidade = Notificacao.query.filter_by(
